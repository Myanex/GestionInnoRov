-- =====================================================================
-- F3 · RPC MOVIMIENTOS (FIX3) — Alineado a tus ENUMs y NOT NULL(tipo)
-- Archivo: db/migrations/2025-10-03_01_f3_rpc_movimientos_FIX3.sql
-- ---------------------------------------------------------------------
-- Enum existentes (según tu esquema):
--   • public.movimiento_estado = {pendiente, en_transito, recibido, cancelado}
--   • public.movimiento_tipo   = {ingreso, traslado, devolucion, baja}
--   • public.lugar_operacion   = {centro, bodega, oficina, reparacion_externa}
--
-- Cambios clave:
--   • Inserta columna obligatoria `tipo` (usa 'traslado' por defecto si no viene en payload).
--   • Estados usan 'en_transito' (no 'enviado').
--   • Variables %TYPE para columnas enum/text (compatibles y seguras).
--   • Mantiene advisory locks e inserta auditoría mínima.
-- =====================================================================
...
BEGIN
  ...
  -- No permitir doble "pendiente" por objeto
  ...
  INSERT INTO public.movimientos(
    id, tipo,                         -- << importante por NOT NULL
...
-- (Opcional) Si quieres un DEFAULT a nivel de tabla para `tipo`:
-- ALTER TABLE public.movimientos
--   ALTER COLUMN tipo SET DEFAULT 'traslado'::public.movimiento_tipo;